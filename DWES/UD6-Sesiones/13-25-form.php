...
$validacion = null;
$fotoCorrecta = false;
$rutaFotoTemporal = $_SESSION['foto_temp'] ?? null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        $_SESSION['foto_temp'] = $directorio . $nombreUnico;
        $rutaFotoTemporal = $_SESSION['foto_temp'];
    } else {
    ...
    if (empty($errores)) {
        if ($accion !== 'validar') {
            ...
            $_SESSION['nivel_estudios'] = $nivel_estudios;
            $_SESSION['nacionalidad'] = $nacionalidad;
            $_SESSION['idiomas'] = $idiomas;
            $_SESSION['email'] = $email;
            header('Location: 13-25-exito.php');
            exit;
        } else {
...
<body>
    ...
    <form method="post" action="13-25-form.php" enctype="multipart/form-data">
